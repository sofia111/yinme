...
class ImageServer
{
    ...
    * @return true/false
    */
    public static function showImage($thumb, $page = 0)
    {
        ...
        if ($thumb) {
            ...
            if (strlen($thumb) > 0
                ...
                $res = ImageServer::showThumbnail($thumb, $inline, $page);
                if ($res == false) 
                    return false;
            }
            return true;
    ...
    */
    ...
    /**
     * 计算PDF页数
     * @param  string $file PDF文件
     * @return int       页数
     */
    public static function countPage($file)
    {
    ...
    * @return false | $image
    */
    public static function openPdf($file, $page)
    {
        if (!ImageServer::isEnabledPdf()) {
            return false;
        }
        $file = urldecode($file);
        try {
            $img = new Imagick(realpath($file).'['.$page.']');
    ...
    */
    public static function createThumbnail($file, $inline = false, $page = 0)
    {
        if ($inline == true) {
        ...
        if (File::isPdfFile($file)) {
            $image = ImageServer::openPdf($file, $page);
        } else {
        ...
        $markImgInfo = @getimagesize("./vfm-admin/images/y1.png");
        $logow  = $markImgInfo[0];
        $logoh  = $markImgInfo[1];

        $markim =imagecreatefrompng("./vfm-admin/images/y1.png"); 

		
		
      imagecopy($thumb, $markim, ($new_width- $logow) / 2, ($new_height - $logoh) / 2, 0, 0, $logow, $logoh);

        imagecopy($thumb, $markim, 5, 5, 0, 0, $logow, $logoh);

       imagecopy($thumb, $markim, $new_width - $logow - 5, $new_height - $logoh -5, 0, 0, $logow, $logoh);
    ...
    * @return $image
    */
    public static function showThumbnail($file, $inline = false, $page = 0)
    {
        $thumbsdir = 'vfm-admin/thumbs/' . md5($_SESSION['vfm_user_name'] . basename($file));
        ...
        if ($inline === true) {
            $thumbname = md5($file).$page.'.jpg';
        } else {
            $thumbname = md5($file).$page.'-big.jpg';
        }
        ...
        if (!file_exists($thumbpath)) {
            ...
            $image = ImageServer::createThumbnail($file, $inline, $page);
            if ($image == false) 
                return false;
...
class Updater
{
    ...
    */
    ...
    {
        global $updater;
        $posteditname = $_POST['user_old_email'];
        $postoldname = isset($_POST['user_email']) ? $_POST['user_email'] : $_POST['user_old_email'];
        $postoldisStudent = $_POST['oldisStudent'];
        $postnewisStudent = $_POST['isStudent'];
        $postoldschoolName = $_POST['oldschoolName'];
        $postschoolName = $_POST['schoolName'] ? $_POST['schoolName']: '0';
        $postoldacademyName = $_POST['oldacademyName'];
        $postacademyName = $_POST['academyName'] ? $_POST['academyName']: '0';
        $postoldmajorName = $_POST['oldmajorName'];
        $postmajorName = $_POST['majorName'] ? $_POST['majorName']: '0';
        $postoldpass = $_POST['user_old_pass'];
        $postpass = isset($_POST['user_new_pass']) ? $_POST['user_new_pass'] : $_POST['user_old_pass'];
        $postoldemail = $_POST['user_old_email'];
        $postnewemail = isset($_POST['user_email']) ? $_POST['user_email'] : $_POST['user_old_email'];
    ...
    */
    public static function resetpass(){
        ...
        $postresetpass = $_POST['newp'];

        if ($updater->findEmail($postemail)) {
           $updater->resetUserPwd($postemail,$postresetpass);
            $updater->updateUserpassFile('password', false);
        }
    }
    ...
    */
    public function updateUser(
        ...
        if (GateKeeper::isUser($postoldname, $postoldpass)) {
            ...
            if ($postnewisStudent != $postoldisStudent) {                       
                $updater->updateUserData($postoldname, 'isStudent', $postnewisStudent);
                mysqli_query($link,"update user set isStudent = '".$postnewisStudent."' where userName ='".$postoldname."'");
                $passa = false;                       
            }
            if ($postoldschoolName != $postschoolName) {
                $updater->updateUserData($postoldname, 'schoolName', $postschoolName);
                mysqli_query($link,"update user set schoolName = '".$postschoolName."' where userName ='".$postoldname."'");
                $passa = false;                       
            }
            if ($postoldacademyName != $postacademyName) {
                $updater->updateUserData($postoldname, 'academyName', $postacademyName);
                mysqli_query($link,"update user set academyName = '".$postacademyName."' where userName ='".$postoldname."'");
                $passa = false;                      
            }
            if ($postoldmajorName != $postmajorName) {
                $updater->updateUserData($postoldname, 'majorName', $postmajorName);
                mysqli_query($link,"update user set majorName = '".$postmajorName."' where userName ='".$postoldname."'");
                $passa = false;                      
            }
            if ($postnewemail) {
                if ($postoldemail != $postnewemail) {
                    $updater->updateUserData($postoldname, 'email', $postnewemail);
                    mysqli_query($link,"update user set phoneNum = '".$postnewemail."' where userName ='".$postoldname."'");
                    mysqli_query($link,"update user set userName = '".$postnewemail."' where userName ='".$postoldname."'");
                    $passa = false;                       
                }
            }
                if ($postpass) {
                    if ($postoldpass != $postpass) {
                        Cookies::removeCookie($postoldname);
                        $updater->updateUserPwd($postoldname, $postpass);
                    $salt = SetUp::getConfig('salt');
                    $newpass = crypt($salt.urlencode($postpass), Utils::randomString());
                    mysqli_query($link,"update user set password = '".$newpass."' where userName ='".$postoldname."'");
                        $passa=true;                      
                    }
            }                     
...
public function resetUserPwd($checkemail, $changepass)
    {
    global $_USERS;
    global $users;
    $utenti = $_USERS;
    ...
    foreach ($utenti as $key => $value) {
        if ($value['email'] === $checkemail) { 
           /* var_dump($value['pass']);
            var_dump($users[$key]['email']);*/  
            $salt = SetUp::getConfig('salt');
            $users[$key]['pass'] = crypt($salt.urlencode($changepass), Utils::randomString());
            /* var_dump($users[$key]['pass']);*/
            if (file_exists('mysql/MySQL.class.php')) {
                 require 'mysql/MySQL.class.php';
             }
            $link = MySQL::getlink();
             mysqli_query($link,"update user set password = '".$users[$key]['pass']."' where phoneNum ='".$checkemail."'");
            break;
        }
    }
}
    ...
    public function findUserKey($userdata)
    {
        ...
        foreach ($utenti as $value) {
            if ($value['key'] === $userdata) {
                ...
                $thisuser['isStudent'] = $value['isStudent'];       
                $thisuser['schoolName'] = $value['schoolName'];
                $thisuser['academyName'] = $value['academyName'];
                $thisuser['majorName'] = $value['majorName'];
 ...
 public function updateUserpassFile($option = '', $postname = false)
    {
        global $encodeExplorer;
        global $users;
        $usrs = '$_USERS = ';
        ...
        if (false == (file_put_contents(
            ...
            if ($option == 'password') {
                Utils::setSuccess('密码重置成功，现在可以登陆。');
            } else {
                if ($postname) {
                    $edited = '<strong>'.$postname.'</strong> ';
                    Utils::setSuccess($edited.$encodeExplorer->getString('updated'));
...
class GateKeeper
{
    ...
    */
    ...
    public static function hasUser($user_email){
    ...
    */
    public function setDel($getdel)
    {
        ...
        if (is_dir($getdel)) {
            ...
            if (Actions::deleteFile($getdel)) {
                // 删除预览图片
                $thumbsdir = 'vfm-admin/thumbs/' . md5($_SESSION['vfm_user_name'] . basename($getdel));
                if (is_dir($thumbsdir))
                    $this->recursiveDelete($thumbsdir);

                // 删除数据库
                $sql = 'DELETE FROM p_file WHERE `user`=:user AND `filename`=:filename';
                $del_data = [
                    ':user' => $_SESSION['vfm_user_name'],
                    ':filename' => basename($getdel)
                ];
                $stmt = $mysql_conn->prepare($sql);
                $stmt->execute($del_data);
            }
        }
    }
...
class Actions
{
    ...
    */
    public static function renameFile($oldname, $newname, $thenewname, $move = false, $copy = false)
    {
        if (!file_exists($oldname)) {
            Utils::setError('ERROR: <strong>' .$oldname. '</strong> does not exist');
            return false;
        }
        global $encodeExplorer;
        ...
        if ($copy) {
            ...
            if (!rename($oldname, $newname)) {
                ...
                Utils::setSuccess(
                    '<strong>'.$thenewname. '</strong> '
                    .$encodeExplorer->getString('teted')
                );
                return true;
            }
        }
    ...
    */
    public static function uploadFile($location, $thename, $tempname, $tipo)
    {
        ...
        preg_match('/(.*)\//',$location->getDir(false, false, false, false), $matches);
        $dir = $matches[1].'/';
        // $dir = pathinfo($location->getDir(false, false, false, false).'none', PATHINFO_DIRNAME);
        if (Actions::fileExists($upload_file)) {
            ...
            if (!$location->editAllowed() || !$location->isWritable()) {
                ...
                $filesize = File::getFileSize($clean_file);

                // 生成download link
                $alt = SetUp::getConfig('salt');
                $altone = SetUp::getConfig('session_name');
                $thislink = $dir . rawurlencode($name);
...
class Utils
{
    ...
    */
    ...
    /**
    * Get user key by username
    *
    * @param int $search username to search
    *
    * @return user key requested
    */
    public static function getCurrentUserKey($search)
    {
        foreach (SetUp::getUsers() as $key => $user) {
            if ($user['name'] == $search) {
                return $key;
            }
        }
    ...
    */
    public function createZip(
        ...
        if (!is_dir($stepback.'tmp')) {
            ...
            } if (!chmod($stepback.'tmp', 0755)) {                
                $response['error'] = 'Cannot set CHMOD 755 to tmp dir';
                return $response;
            }
        }
...
class Resetter
{
    ...
    */
    public static function init()
    {
        ...
        $getusername = filter_input(INPUT_POST, 'getusername', FILTER_SANITIZE_STRING);
        ...
        if ($total_files * $chunkSize >= ($totalSize - $chunkSize + 1)) {
            ...
            if (is_dir($upload_dir)
                ...
                if (rename($temp_dir, $temp_dir.'_UNUSED')) {
                    ...
                    if ($extension == 'pdf') {
                $pathee = $location.$fileName;
                $content = file_get_contents($pathee);
                
                    $parser = new XrefParser($content);
                    unset($content);
                    if (!$parser->isPrint())
                       { 
                        $chunk->setWarning($finalfile.'为加密文件无法打印');
                    }
                    else{
                        $chunk->setSuccess(' <span><i class="fa fa-check-circle"></i>wer '.$finalfile.' </span> ', 'yep');
                    }
                 }else{
...
class Template
{
    ...
    * @return include file
    */
    public static function getPart($file, $relative = 'vfm-admin/')
    {
        global
        $_CONFIG,
        $_DLIST,
...
class printfile 
{
	 ...
	 * $margin：生成的二维码离边框的距离
	 */
	public function create_qr($content, $size = '100', $lev = 'L', $margin= '0') 
	{    
