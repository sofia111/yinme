    ...
    if ($gateKeeper->isAllowed('upload_enable')) { 
        ...
        <form enctype="multipart/form-data" method="post" id="upForm" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']);?>">
            ...
            <div id="upload_container" class="input-group pull-left <?php echo $class; ?>">
                ...
                <div style="text-align: center; font-size: 20px; width: 50%;height:50px;line-height: 50px;position:fixed;left:0px;bottom:0px;background:#448AFF;z-index: 1;border-right:solid 1px #fff; " class="upload_sumbit visible-xs" id="upformsubmit" disabled>
                    <a id="upchunk" style="font-size: 20px; color: #fff;" href="javascript:void(0);" ><i class="fa fa-upload fa-fw"></i>上传</a></div><div style="text-align: center; font-size: 20px; width: 50%;height:50px;line-height: 50px; position:fixed;right:0px;bottom:0px;background:#448AFF;z-index: 1;border-left:solid 1px #fff; " class="visible-xs"><a id="creatorderlist" href="javascript:void(0);" ><span style="color: #fff;"><i class="fa fa-money"></i>&nbsp&nbsp下单</a></span>
                    </div>
            </div>
        </form>
