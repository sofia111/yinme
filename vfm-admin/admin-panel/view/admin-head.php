...
if (isset($_GET['languagemanager'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if($_GET['shops'] == "new") {
            ...
            $shopschool = filter_input(
                INPUT_POST, "shopschool", FILTER_SANITIZE_STRING
            );
            $shopname = filter_input(
                INPUT_POST, "shopname", FILTER_SANITIZE_STRING
            );
            $onesideprice = filter_input(
                INPUT_POST, "onesideprice", FILTER_SANITIZE_STRING
            );
            $bothsideprice = filter_input(
                INPUT_POST, "bothsideprice", FILTER_SANITIZE_STRING
            );
            $shopaddress = filter_input(
                INPUT_POST, "shopaddress", FILTER_SANITIZE_STRING
            );
            $bindprice = filter_input(
                INPUT_POST, "bindprice", FILTER_SANITIZE_STRING
            );
			$deliveryprice = filter_input(
                INPUT_POST, "deliveryprice", FILTER_SANITIZE_STRING
            );


		$mysql_conn=MySQL::getConn();
        $query_data = [
            ':shopschool' =>$shopschool,
            ':shopname' =>$shopname,
            ':onesideprice' =>$onesideprice,
            ':bothsideprice' =>$bothsideprice,
    ...
    $newshoper = array();
    $newshoper['name'] = $shopname;
    $newshoper['address'] = $shopaddress;
    $newshoper['bind_price'] = $bindprice;
    $newshoper['onesideprice']= $onesideprice;
    $newshoper['bothsideprice'] = $bothsideprice;
    $newshoper['school'] = $shopschool;
	$newshoper['deliveryprice'] =$deliveryprice; 	
    
    if ($_SHOPS == null) {
        $_SHOPS =array();
       array_push($_SHOPS, $newshoper);
        
    }else{
		array_push($_SHOPS, $newshoper);
	}  
    file_put_contents('./users/shops.php', "<?php\n\n $shops".var_export($_SHOPS,true).";\n");
    
    }   
  }
}elseif (isset($_GET['orderlist'])) {
    $activesec = "orderlist";
 ...
 elseif (isset($_GET['log'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        */
        ...
        $_CONFIG['direct_links'] = (isset($_POST['direct_links']) ? true : false);
        ...
        */
        ...
        if ($updater->updateHtaccess($_CONFIG['starting_dir'], $_CONFIG['direct_links']) == false) {
